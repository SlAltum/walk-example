...
var ListBoxOwnerDrawingCancel chan bool = make(chan bool, 1)

func NewListBoxOwnerDrawingMainWindow() *MainWindow {
	...
	listBoxOwnerDrawingMw := MainWindow{
		AssignTo: &mw,
	...
	if err := listBoxOwnerDrawingMw.Create(); err != nil {
		log.Fatal(err)
	}
	...
	go func() {
		for {
			...
			case <-ListBoxOwnerDrawingCancel:
				break
			}
		}
	}()
